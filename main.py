...
from utils.formatters import format_analysis_report, save_analysis_report, format_console_report
...
Examples:
  ...
  python main.py --bug-report bug.json --repo owner/repo --output reports/analysis.json
    ...
    parser.add_argument(
        '--repo', 
        required=True, 
        help='GitHub repository (owner/repo-name)'
    )
    parser.add_argument(
        '--branch', 
        default='main', 
    ...
    try:
        ...
        try:
            ...
            logger.info(f"Connected to {repo_info.full_name} ({repo_info.default_branch})")
